...
export default function GroupPredictionsPage({ onCountChange }: { onCountChange?: (n: number) => void }) {
  ...
  // Auto-advance: track when the active group was last completed to avoid re-triggering
  const autoAdvancedRef = useRef<string | null>(null)
  const prevSavedCountRef = useRef(0)

  useEffect(() => {
    if (loading || matches.length === 0) return
    // Only react when savedCount increases (a new prediction was just saved)
    if (savedCount <= prevSavedCountRef.current) { prevSavedCountRef.current = savedCount; return }
    prevSavedCountRef.current = savedCount

    const groupMatchIds = matches.filter(m => m.group_letter === activeGroup).map(m => m.id)
    const filledCount = groupMatchIds.filter(id =>
      savedIds.has(id) && preds[id]?.home !== '' && preds[id]?.away !== ''
    ).length
    if (filledCount < groupMatchIds.length || autoAdvancedRef.current === activeGroup) return
    autoAdvancedRef.current = activeGroup

    const nextIncomplete = GROUPS.find(g => {
      if (g === activeGroup) return false
      const ids = matches.filter(m => m.group_letter === g).map(m => m.id)
      return ids.filter(id => savedIds.has(id) && preds[id]?.home !== '' && preds[id]?.away !== '').length < ids.length
    })
      ...
      <div className="grid lg:grid-cols-[1fr_300px] gap-6 items-start">
              ...
              <div key={match.id} className={`bg-white rounded-xl shadow-sm p-4 border-l-4 ${hasError ? 'border-red-400' : matchLocked ? 'border-gray-300' : 'border-transparent'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0 flex items-center justify-end gap-1.5 overflow-hidden">
                    ...
                    {homeCode && <a href={`/teams/${homeCode}`}><img src={flagUrl(homeCode, 40)} alt="" className="w-7 h-5 object-cover rounded-sm flex-shrink-0 hover:opacity-80" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} /></a>}
                  </div>
                  {/* Inputs */}
                  ...
                  <div className="flex-1 min-w-0 flex items-center gap-1.5 overflow-hidden">
                    {awayCode && <a href={`/teams/${awayCode}`}><img src={flagUrl(awayCode, 40)} alt="" className="w-7 h-5 object-cover rounded-sm flex-shrink-0 hover:opacity-80" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} /></a>}
                    {awayCode ? (
          ...
          {standings.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm overflow-hidden mt-2">
              <div className="bg-[#0B1F3A] text-white px-4 py-2.5 text-sm font-bold flex items-center gap-2">
                {t('grp_ko_preview')}
                <span className="text-white/60 font-normal text-xs">{t('grp_based_on')}</span>
              </div>
                    ...
                    return (
                      <div key={s.team.id} className="px-4 py-3">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {teamCode && <img src={flagUrl(teamCode, 40)} alt="" className="w-6 h-4 object-cover rounded-sm flex-shrink-0" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} />}
                        ...
                        {qualifies && (
                          <div className="ml-7 space-y-1">
                            <div className="text-xs text-gray-500 flex items-center gap-1.5">
                              ...
                              {r32OppCode && <img src={flagUrl(r32OppCode, 40)} alt="" className="w-4 h-3 object-cover rounded-sm" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} />}
                            ...
                            <div className="text-xs text-gray-400 flex items-center gap-1.5 flex-wrap">
                              ...
                              {r16Teams.length >= 2 ? (
                                <>
                                  ...
                                  {(r16Teams[0] as any).fifa_code && <img src={flagUrl((r16Teams[0] as any).fifa_code, 40)} alt="" className="w-4 h-3 object-cover rounded-sm" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} />}
                                  <span className="font-medium text-gray-600">{tn((r16Teams[0] as any).fifa_code, r16Teams[0].name)}</span>
                                  <span>{t('grp_or')}</span>
                                  {(r16Teams[1] as any).fifa_code && <img src={flagUrl((r16Teams[1] as any).fifa_code, 40)} alt="" className="w-4 h-3 object-cover rounded-sm" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} />}
        ...
        <div className="lg:sticky lg:top-24 flex flex-col gap-4">
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="bg-[#0B1F3A] text-white px-4 py-2.5 text-sm font-bold">{t('grp_standings', { g: activeGroup })}</div>
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-gray-100 text-gray-400">
                  <th className="px-3 py-2 text-left">{t('grp_team')}</th>
                  <th className="px-2 py-2 text-center">{t('grp_p')}</th>
                  <th className="px-2 py-2 text-center">{t('grp_w')}</th>
                  <th className="px-2 py-2 text-center">{t('grp_d')}</th>
                  <th className="px-2 py-2 text-center">{t('grp_l')}</th>
                  <th className="px-2 py-2 text-center">{t('grp_gd')}</th>
                  <th className="px-2 py-2 text-center font-bold text-[#0B1F3A]">{t('grp_pts')}</th>
                </tr>
              </thead>
              <tbody>
                {standings.map((s, i) => (
                  <tr key={s.team.id} className={`border-t border-gray-50 ${i < 2 ? 'bg-green-50' : i === 2 ? 'bg-yellow-50' : ''}`}>
                    <td className="px-3 py-2 flex items-center gap-1.5">
                      ...
                      {(s.team as any).fifa_code && (
                        <img src={flagUrl((s.team as any).fifa_code, 40)} alt="" className="w-5 h-3.5 object-cover rounded-sm" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} />
                      )}
            ...
            <div className="px-3 py-1.5 border-t border-gray-100 flex gap-3 text-[10px] text-gray-400">
              <span><span className="inline-block w-2 h-2 bg-green-200 rounded-sm mr-1" />{t('grp_advance')}</span>
              <span><span className="inline-block w-2 h-2 bg-yellow-200 rounded-sm mr-1" />{t('grp_best3rd')}</span>
            </div>
          </div>
          ...
          {best3rds.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="bg-[#0B1F3A] text-white px-4 py-2.5 text-sm font-bold">{t('grp_best3rd_title')}</div>
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-100 text-gray-400">
                    <th className="px-3 py-2 text-left">{t('grp_team')}</th>
                    <th className="px-2 py-2 text-center">{t('grp_grp')}</th>
                    <th className="px-2 py-2 text-center">{t('grp_gd')}</th>
                    <th className="px-2 py-2 text-center font-bold text-[#0B1F3A]">{t('grp_pts')}</th>
                  </tr>
                </thead>
                    ...
                    <tr key={r.team.id} className={`border-t border-gray-50 ${r.qualifies ? 'bg-green-50' : ''}`}>
                      <td className="px-3 py-2 flex items-center gap-1.5">
                        ...
                        {(r.team as any).fifa_code && (
                          <img src={flagUrl((r.team as any).fifa_code, 40)} alt="" className="w-5 h-3.5 object-cover rounded-sm" onError={e => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden' }} />
                        )}
              ...
              <p className="text-[10px] text-gray-400 px-3 py-1.5 border-t border-gray-100">{t('grp_based_so_far')}</p>
            </div>
          )}
        </div>
      </div>
