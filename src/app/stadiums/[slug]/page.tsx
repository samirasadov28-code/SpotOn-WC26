  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {groupMatches.length > 0 && (
        <div className="mb-10">
          <h2 className="text-lg font-black text-[#0B1F3A] mb-4">⚽ Group Stage Matches</h2>
          <div className="flex flex-col gap-3">
            {groupMatches.map(match => {
              const home = teamByCode.get(match.home_team)
              const away = teamByCode.get(match.away_team)
              return (
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      {home ? (
                        ...
                        <span className="font-semibold text-[#0B1F3A] text-sm">{match.home_team}</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
                      {away ? (
                        ...
                        <span className="font-semibold text-[#0B1F3A] text-sm text-right">{match.away_team}</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {koMatches.length > 0 && (
        <div className="mb-10">
          <h2 className="text-lg font-black text-[#0B1F3A] mb-4">🏆 Knockout Matches</h2>
          <div className="flex flex-col gap-3">
            {koMatches.map(match => {
              ...
              const homeLabel = home ? home.name : match.home_team ? match.home_team : 'TBD'
              const awayLabel = away ? away.name : match.away_team ? match.away_team : 'TBD'
              return (
                <div key={match.id} className="bg-white border border-gray-100 rounded-2xl px-4 py-4 shadow-sm">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
      ...
      {(matches ?? []).length === 0 && (
        <div className="mb-10 bg-gray-50 border border-gray-200 rounded-2xl p-6 text-center text-gray-500 text-sm">
          Match schedule for this venue will appear here once confirmed.
        </div>
      )}
